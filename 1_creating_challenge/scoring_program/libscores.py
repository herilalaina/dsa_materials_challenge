...
def get_metric():
    ...
    with open(_HERE('metric.txt'), 'r') as f:
        for metric_name in f:
            ...
            list_metric.append((metric_name, scoring_function))
...
def sanitize_array(array):
    ...
    a = np.ravel(array)
    maxi = np.nanmax(list(filter(lambda x: x != float('inf'), a)))  # Max except NaN and Inf
    mini = np.nanmin(list(filter(lambda x: x != float('-inf'), a)))  # Mini except NaN and Inf
...
def normalize_array(solution, prediction):
    ...
    maxi = np.nanmax(list(filter(lambda x: x != float('inf'), sol)))  # Max except NaN and Inf
    mini = np.nanmin(list(filter(lambda x: x != float('-inf'), sol)))  # Mini except NaN and Inf
    if maxi == mini:
...
def mvmean(R, axis=0):
    ...
    if axis == 1:
        return np.array(list(map(average, R)))
    else:
        return np.array(list(map(average, R.transpose())))
...
def auc_binary(solution, prediction):
    ...
    return metrics.roc_auc_score(solution, prediction)
